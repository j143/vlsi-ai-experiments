...
_REAL_CSV = Path(__file__).parent.parent / "datasets" / "bandgap_sweep_real_sky130.csv"


@pytest.mark.skipif(not _REAL_CSV.exists(), reason="Real SKY130 dataset not present")
class TestSurrogateAccuracyRealData:
    """Accuracy tests on the reference SKY130 analytical sweep dataset.

    The dataset (600 rows, 3 corners × 200 LHS) was generated by running the
    analytical Brokaw formula over Latin-Hypercube samples.  A GP trained on
    320 of those rows should predict the remaining 60 held-out rows to within
    ±10 mV with ≥80% accuracy (High or Medium confidence label).
    """
    ...
    def _load_split(self, seed: int = 42):
        import pandas as pd
        from ml.surrogate import FEATURES  # noqa: PLC0415

        df = pd.read_csv(_REAL_CSV)
        df = df.dropna(subset=["vref_V"])
        df = df[df["vref_V"].between(0.3, 3.5)]
    ...
    def test_dataset_has_enough_rows(self):
        import pandas as pd

        df = pd.read_csv(_REAL_CSV)
        assert len(df) >= self._N_TRAIN + self._N_TEST, (
            f"Dataset too small: {len(df)} rows < {self._N_TRAIN + self._N_TEST} required"
        )
    ...
    def test_real_data_accuracy_within_tolerance(self, capsys):
        ...
        print(
            f"\n[Real SKY130] Surrogate accuracy (within ±{self._TOLERANCE_MV:.0f} mV): "
            f"{accuracy * 100:.1f}% ({int(accuracy * self._N_TEST)}/{self._N_TEST} pts)  "
            f"Mean error: {errors_mV.mean():.2f} mV  "
    ...
    def test_real_data_confidence_is_not_low(self):
        ...
        assert confidence in ("High", "Medium"), (
            f"Real-data confidence '{confidence}' is too low "
    ...
    def test_real_data_mean_error_below_threshold(self):
        ...
        mean_err_mV = float(np.abs(mean - y_test).mean() * 1000)
        assert mean_err_mV < 50.0, (
            f"Real-data mean error {mean_err_mV:.1f} mV > 50 mV threshold"
        )
